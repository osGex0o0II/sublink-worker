...
describe('Issue #334: rule-provider key collision fix', () => {
  ...
  it('rules should reference correct provider keys', async () => {
    ...
    // Non-China should still work and route through Auto Select without its own group
    expect(config.rules).toContainEqual(expect.stringMatching(/^RULE-SET,geolocation-!cn,.*自动选择/));
  });
